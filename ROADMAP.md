...
| Modern application framework | first slice implemented | New-name Zap successor: typed App/endpoints, raw parameters, explicit decoding, flat uploads, bounded responses and 20 supported example ports. Public middleware/resumable APIs and separate-repository extraction with a pinned engine dependency remain. [App API roadmap](docs/APP-API-ROADMAP.md). |
| Framework-owned std.Io | deferred | Caller-supplied std.Io + current custom io_uring/kqueue engine ships first by user decision. [Exploration and later prototype gates](docs/STD-IO-DECISION.md) preserve the optional owned runtime direction. |
