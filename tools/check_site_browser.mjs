...
try {
  ...
  await check('desktop-landing', async () => {
    ...
    assert.equal(state.title,'Baz — Bounded Async Zap'); assert.equal(state.diagrams,4); assert.equal(state.examples,23); assert(state.highlight);
  ...
  await check('example-filters', async () => {
    ...
    for (const [group,count] of [['data',2],['composition',6],['app',3],['routing',5],['responses',7],['all',23]]) {
      ...
      assert.equal(await evaluate('document.querySelector("#example-count").textContent'),`${count} of 23 examples`);
    }
    return {groups:6,total:23};
  });
  ...
  await check('print-and-no-javascript', async () => {
    ...
    await navigate();
    assert.equal(await evaluate('document.querySelectorAll(".example-card:not([hidden])").length'),23);
    ...
    await send('Emulation.setScriptExecutionDisabled',{value:false}); return {printBytes:Buffer.from(pdf.data,'base64').length,noJsExamples:23};
  });
