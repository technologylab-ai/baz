//! Middleware fills typed request locals before the handler runs.
...
pub const user_middleware: Application.Middleware = .{ .before = UserMiddleware.before };
pub const session_middleware: Application.Middleware = .{ .before = SessionMiddleware.before };

pub const UserMiddleware = struct {
    pub fn before(ctx: *Application.Context) !Application.Decision {
        ...
        ctx.locals.user = .{ .name = "renerocksai", .email = "supa@secret.org" };
...
pub const SessionMiddleware = struct {
    pub fn before(ctx: *Application.Context) !Application.Decision {
        ...
        ctx.locals.session = .{ .info = "secret session", .token = "rot47-asdlkfjsaklfdj" };
        return .continue_request;
    }
};

pub fn render(ctx: *Application.Context) !void {
