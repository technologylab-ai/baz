...
const Options = struct {
    ...
    max_response: usize = 16 * 1024 * 1024,
    timeout_ms: u32 = 5000,
    ...
    pub const help =
        \\ Streaming: GET /stream; curl --no-buffer http://localhost:8080/stream
        \\
...
pub fn main(init: std.process.Init) !void {
    ...
    if (options.stall_ms > 5000 or options.response_body > 8192) return error.InvalidArgument;
    var shared: Shared = .{ .stall_ms = options.stall_ms, .chunk_bytes = options.response_body };
