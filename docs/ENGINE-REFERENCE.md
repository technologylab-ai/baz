# zig-http engine reference

This document preserves the engine's commands, deployment limits and ownership
model while the App framework develops in this worktree. Start with the
[framework README](../README.md) and [App guide](APP-API.md) for the public API.
...
To use the low-level engine, import the `bounded_http` module exported by
...
`begin` copies its `content_type` into the response head. There is no dynamic lease-release callback
