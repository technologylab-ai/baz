...
[`App.init`](https://technologylab-ai.github.io/baz/api/#baz.App.AppWithLocals.init) allocates a stable App owner and bounded registration storage.
...
Register routes before [`App.start`](https://technologylab-ai.github.io/baz/api/#baz.App.AppWithLocals.start).
Startup prepares engine threads and buffers behind the request-processing barrier.
Baz seals its allocator before [`App.run`](https://technologylab-ai.github.io/baz/api/#baz.App.AppWithLocals.run) releases that barrier.
...
| `print`, `jsonValue`, [`mustache`](https://technologylab-ai.github.io/baz/api/#baz.mustache) | Generate directly into the reserved draft without an intermediate allocated string; move the generated bytes once for publication. |
...
| Write → flush → borrow an image → continue streaming | **Not supported by Baz’s current API.** There is no borrowed-write operation on [`Stream`](https://technologylab-ai.github.io/baz/api/#baz.Stream). |
...
[`Response`](https://technologylab-ai.github.io/baz/api/#baz.Response) can call [`Response.initWithLimit(writer, limits, max_response_bytes)`](https://technologylab-ai.github.io/baz/api/#baz.Response.initWithLimit);
[`Response.init(writer, limits)`](https://technologylab-ai.github.io/baz/api/#baz.Response.init) retains its conservative `limits.body_bytes`
...
[`stream.finish()`](https://technologylab-ai.github.io/baz/api/#baz.Stream.finish) alone is not permission to release externally borrowed memory.
