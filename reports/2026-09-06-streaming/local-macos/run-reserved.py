...
token=str(uuid.uuid4());owner={'agent':'baz streaming /root','purpose':sys.argv[1],'host':socket.gethostname(),'started_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'owner_pid':os.getpid(),'token':token,'cwd':str(root)}
(lock/'owner.json').write_text(json.dumps(owner,indent=2)+'\n')
out=root/'.zig-cache/streaming-evidence';out.mkdir(exist_ok=True);log=out/(sys.argv[1]+'.log');proc=None;code=1
try:
